...
struct HomeView: View {
    ...
    @State private var goPropertySearch = false
        ...
        NavigationStack {
            ZStack {
                ...
                ScrollView(.vertical, showsIndicators: false) {
                    VStack(spacing: 0) {
                        VStack(spacing: 24) {
                            HomeHeaderView(onTapSearchArea: { goPropertySearch = true })
                                .padding(.top, 10)
            ...
            .navigationDestination(isPresented: $goPropertySearch) {
                PropertySearchView()
            }
...
// MARK: - 검색 전용 페이지 (HomeHeader 탭 진입)
